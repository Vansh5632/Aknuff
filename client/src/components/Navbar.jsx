  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={navbarVariants} className="sticky top-0 z-50 w-full">
          ...
          <div className="flex items-center space-x-4">
            <motion.div className="flex items-center space-x-3" variants={itemVariants}>
              ...
              >
                <Bell size={18} />
                {notificationCount > 0 && (
                  <motion.div className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                    {notificationCount}
                  </motion.div>
                )}
              </motion.a>
          ...
          >
            <div className="flex flex-col h-full">
              ...
              <motion.div className="mt-auto">
                <motion.button className="w-full bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-700 transition-colors duration-300">
                  Contact Support
                </motion.button>
              </motion.div>
            </div>
      ...
      <motion.nav
        ...
        style={{ top: scrolled ? '3.5rem' : '5rem' }}
      >
          ...
          {['All Categories', 'Steam Gift Cards', 'BIGO Live', 'PSN Card', 'RBL Universe', 'Top Up MLBB', 'Game Points'].map((category, index) => (
            ...
            >
              <span className="relative">
                {category}
                <motion.span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-indigo-500" whileHover={{ width: "100%" }} transition={{ duration: 0.3 }} />
              </span>
